...
class AuthController {
  ...
  Future<Map<String, dynamic>> getCalendar() async {
    ...
    try {
      final response = await client.getCalendar();
      //check if response contains message
      ...
      log("failed exception happened during getCalendar: $e");
      return {
        "error": "Failed to get calendar",
        "status": "error",
      };
  ...
  //get maturities
  Future<Map<String, dynamic>> getMaturities() async {
    final dio = Dio();
    final client = RestClient(dio);
    dio.options.headers['Authorization'] = "Bearer ${await getAccessToken()}";
    //accept application/json
    ...
    try {
      final response = await client.getMaturities();
      //check if response contains message
      if (response.containsKey('message')) {
                  return {
            "maturities": response['data'],
            "status": "success",
          };
        ...
        log('failed response: $response');
        return {
          "error": "Failed to get maturities",
          "status": "error",
        };
      }
    } catch (e) {
      log("failed exception happened during getMaturities: $e");
      return {
        "error": "Failed to get maturities",
        "status": "error",
      };
